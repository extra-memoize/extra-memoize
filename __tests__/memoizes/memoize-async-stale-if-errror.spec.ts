import { memoizeAsyncStaleIfError } from '@memoizes/memoize-async-stale-if-error'
...
describe('memoizeAsyncStaleIfError', () => {
  ...
  , ['async cache', SIEAsyncCache]
    ...
    it('caches the result', async () => {
      ...
      const memoizedFn = memoizeAsyncStaleIfError({ cache, verbose: true }, fn)
    ...
    describe('stale-if-error', () => {
      test('caches the result', async () => {
        ...
        expect(result2).toBePromise()
        expect(proResult2).toStrictEqual([1, State.Hit])
        expect(result3).toBePromise()
        expect(proResult3).toStrictEqual([2, State.StaleIfError])
      ...
      describe('fn throws errors', () => {
        it('returns stale cache', async () => {
          ...
          expect(proResult1).toStrictEqual([1, State.Miss])
          expect(result2).toBePromise()
          expect(proResult2).toStrictEqual([1, State.StaleIfError])
          expect(result3).toBePromise()
          expect(proResult3).toStrictEqual([2, State.StaleIfError])
          expect(result4).toBePromise()
          expect(proResult4).toStrictEqual([2, State.Hit])
