import { memoizeStaleWhileRevalidateAndStaleIfErrorWithAsyncCache } from '@memoizes/stale-while-revalidate-and-stale-if-error/memoize-stale-while-revalidate-and-stale-if-error-with-async-cache'
import { State } from '@src/types'
import { SWRAndSIEAsyncCache } from '@test/utils'
...
`, () => {
  it('caches the result', async () => {
    ...
    const cache = new SWRAndSIEAsyncCache(() => State.Hit)

    const memoizedFn = memoizeStaleWhileRevalidateAndStaleIfErrorWithAsyncCache({ cache }, fn)
  ...
  describe('stale-while-revalidate', () => {
    describe('caches the result', () => {
      it('return stale cache then revalidate in background', async () => {
        ...
        const cache = new SWRAndSIEAsyncCache(
          jest.fn()
